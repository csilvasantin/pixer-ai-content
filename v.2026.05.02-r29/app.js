(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r29';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r29';
  ...
  function bindSendToAdmiraXP() {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        const data = await r.json();
        if (!r.ok || !data.ok) {
          showToast('ERROR: ' + (data.error || r.status));
          console.error(data);
        } else {
          showToast('📺 enviado a Admira XP · id ' + data.id.slice(-6));
        }
      } catch (e) {
        showToast('ERROR: ' + String(e).slice(0, 80));
      } finally {
        btn.disabled = false;

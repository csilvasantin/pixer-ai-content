(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r32';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r32';
  ...
  async function refreshXtoreStatus() {
    ...
    try {
      ...
      if (online > 0) {
        ...
        const youngest = (data.screens || []).filter(s => s.online).sort((a, b) => a.age_seconds - b.age_seconds)[0];
        el.textContent = `XTORE · ${online} live`;
        el.title = `${online}/${total} pantallas activas · última señal hace ${youngest?.age_seconds || 0}s\n\n` +
          (data.screens || []).map(s => `${s.online ? '🟢' : '🔴'} ${s.screen} · ${s.age_seconds}s · feed:${s.feed_count}`).join('\n');
      } else if (total > 0) {

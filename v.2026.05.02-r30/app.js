(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r30';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r30';
  ...
  const MOTORES = {
    ...
    musica: [
      ...
      { id: 'lyria-3-clip-preview', nombre: 'Lyria 3 Clip (Google)',  tipo: 'pro',  coste: 'paid tier Gemini',    desc: '30s MP3 con voz cantando la letra' },
      { id: 'lyria-3-pro-preview',  nombre: 'Lyria 3 Pro (Google)',   tipo: 'pro',  coste: 'paid tier Gemini',    desc: '~2min con voz cantando la letra' },
      { id: 'lyria-002',            nombre: 'Lyria 2 (Google)',       tipo: 'pro',  coste: '~$0.06 / sample',     desc: '60s estéreo instrumental (sin letra)' },
    ],
    imagenes: [
    ...
    video: [
      ...
      { id: 'veo-3.0-fast-generate-001', nombre: 'Veo 3 Fast (Google)', tipo: 'pro', coste: '~$0.10 / segundo',        desc: '720p · 4-8s · audio nativo' },
      { id: 'veo-3.0-generate-001',    nombre: 'Veo 3 (Google)',       tipo: 'pro',  coste: '~$0.40 / segundo',       desc: '720p/1080p · audio + diálogos' },
      { id: 'grok-imagine-video',      nombre: 'Grok Imagine (xAI)',   tipo: 'pro',  coste: 'vía worker',             desc: '720p · 1-15s · sin audio' },
  ...
  const SUNO_LOCAL_URL = 'http://localhost:3777';
  ...
  async function sunoLocalAlive() {
    try {
      ...
      return { ok: false, error: 'unreachable: arranca node server.js en suno-local' };
    }
  }
  ...
  async function playSunoLocal(s, model) {
    ...
    const isInstrumental = !s.versiones || /instrumental|loop|bed/i.test(s.versiones);

    const health = await sunoLocalAlive();
    if (!health.ok) {
      showPlayer(`<div class="player-card"><div class="player-head">▶ MÚSICA · Suno · proxy local NO responde</div><pre class="player-body">${health.error}\n\nArranca:\n  cd ~/Documents/New\\ project/csilvasantin-repos/suno-local\n  node server.js</pre></div>`);
      return;
    }
    ...
    try {
      const r = await fetch(SUNO_LOCAL_URL + '/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, instrumental: isInstrumental, model }),
      });
      if (!r.ok) {
        stop(false);
            ...
            <div class="player-card">
              ...
              ${ready.map((c, i) => `
                <div style="display:grid;gap:6px;margin-bottom:10px;">
                  <strong style="color:var(--matrix);text-shadow:var(--glow);">[${i + 1}] ${c.title || 'sin título'} · ${c.metadata?.duration || ''}s</strong>
                  <audio controls src="${c.audio_url}" style="width:100%;"></audio>
                  ${c.image_url ? `<img src="${c.image_url}" style="width:100%;max-height:240px;object-fit:cover;border:1px solid var(--matrix);box-shadow:0 0 12px rgba(0,255,65,.3);">` : ''}
                </div>`).join('')}
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
        ...
        if (poll.done) {
          ...
          showPlayer(`
            <div class="player-card">
              ...
              <video controls autoplay src="${proxyUrl}" style="width:100%; max-height:55vh; border:1px solid var(--matrix); box-shadow:0 0 24px rgba(0,255,65,.30);"></video>
              <pre class="player-body">${prompt.replace(/</g,'&lt;')}</pre>
  ...
  function detectLatestAsset() {
    ...
    if (video) return { kind: 'video', src: video.src };
    const audio = player.querySelector('audio[src]');
    if (audio) return { kind: 'audio', src: audio.src };
    const img = player.querySelector('img[src]');
  ...
  function bindSendToAdmiraXP() {
    ...
    btn.addEventListener('click', async () => {
      ...
      const title = `${page} · ${cliente}`;
      const oldText = btn.textContent;
      btn.disabled = true;
      btn.textContent = '📤 enviando...';

      // Stage 0 — preview en el panel
      const thumbUrl = asset.kind === 'image' ? asset.src : '';
      setSignageStatus({
        thumb: thumbUrl,
        stage: '📤 Detectando contenido',
        log: `${asset.kind.toUpperCase()} · ${page} · "${cliente}"`,
        pct: 5,
        indeterminate: false,
        id: '',
        mode: 'reset',
      ...
      try {
        ...
        // Stage final — verificar feed
        setSignageStatus({ stage: '🔎 Verificando en feed', log: 'comprobando que AdmiraXP puede leerlo', pct: 90, id: data.id });
        try {
          const fr = await fetch(SIGNAGE_URL + '/feed?limit=10');
          const fd = await fr.json();
          const inFeed = (fd.items || []).some(i => i.id === data.id);
          if (inFeed) {
            setSignageStatus({
              stage: '📺 Visible en Admira XP',
              log: `feed total: ${(fd.items || []).length} items · auto-mostrar cuando AdmiraXP lo polee`,
              pct: 100,
              id: data.id,
              mode: 'done',
            });
          } else {
            setSignageStatus({ stage: '⚠ Subido pero no en feed', log: 'raro: aparecerá tras siguiente refresh', pct: 100, mode: 'error' });
          }
        } catch {
          setSignageStatus({ stage: '✓ Subido (feed sin verificar)', log: data.id, pct: 100, id: data.id, mode: 'done' });
        }
      } catch (e) {

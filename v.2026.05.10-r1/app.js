(function () {
    ...
    document.querySelectorAll('[data-motor-section]').forEach(host => {
      ...
      function renderWarning() {
        ...
        const keyLabel = motor.id === 'elevenlabs-v2' ? 'WORKER pixer-eleven'
                       ...
                       : motor.id === 'suno-local-v45' ? 'PROXY suno-local:3777'
  ...
  function showPlayer(html) {
    ...
    p.hidden = false;
    p.innerHTML = html;
    injectXpaceSendButton(p);
  }

  // Bridge capa 1 → 2: botón "Enviar a XpaceOS" en cualquier asset generado.
  // Detecta el primer <img>/<audio>/<video> con src http(s) y lo empuja al
  // worker pixer-eleven (signage feed) que la Xtore polea cada 5s.
  const XPACE_WORKER = 'https://pixer-eleven.csilvasantin.workers.dev';
  function injectXpaceSendButton(container) {
    const media = container.querySelector('img[src^="http"], audio[src^="http"], video[src^="http"]');
    if (!media) return;
    if (container.querySelector('.xpace-send-btn')) return;
    const tag = media.tagName.toLowerCase();
    const kind = tag === 'img' ? 'image' : (tag === 'audio' ? 'audio' : 'video');
    const src = media.getAttribute('src');
    const title = media.dataset.pixerTitle || ('Pixer · ' + kind);
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'xpace-send-btn';
    btn.textContent = '📡 Enviar a XpaceOS';
    btn.title = 'Publica este asset al feed de signage que consumen las Xtores (capa 2)';
    btn.addEventListener('click', async () => {
      btn.disabled = true;
      btn.textContent = '⏳ Enviando…';
      try {
        const r = await fetch(XPACE_WORKER + '/signage/push', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ kind, src, title }),
        });
        const d = await r.json().catch(() => ({}));
        if (r.ok && d.ok) {
          btn.textContent = '✅ Enviado · #' + String(d.id || '').slice(-6);
          btn.classList.add('ok');
        } else {
          btn.textContent = '❌ ' + (d.error || ('HTTP ' + r.status));
          btn.classList.add('err');
          btn.disabled = false;
        }
      } catch (e) {
        btn.textContent = '❌ ' + (e.message || 'fetch error');
        btn.classList.add('err');
        btn.disabled = false;
      }
    });
  ...
  async function playAudio() {
    ...
    if (motor === 'elevenlabs-v2') {
      if (!(await confirmPro('ElevenLabs v2', '$300 / 1M caracteres · vía worker pixer-eleven'))) return;
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
  ...
  async function playVeo(s) {
    const model = 'veo-3.0-generate-001';
    const label = 'Veo 3';
    const costPerSec = 0.40;

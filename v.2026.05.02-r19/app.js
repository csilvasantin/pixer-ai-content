(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r19';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r19';
  ...
  function hasKeyFor(motorId) {
    ...
    if (motorId === 'grok-2-image' || motorId === 'grok-imagine-video') return !!k.xai;
  ...
  const MOTORES = {
    ...
    video: [
      ...
      { id: 'grok-imagine-video', nombre: 'Grok Imagine (xAI)',   tipo: 'pro',  coste: 'requiere XAI_API_KEY',    desc: 'video 720p · 1-15s · async' },
    ...
    document.querySelectorAll('[data-motor-section]').forEach(host => {
      ...
      function renderWarning() {
        ...
        const keyLabel = motor.id === 'elevenlabs-v2' ? 'WORKER pixer-eleven'
                       : (motor.id === 'grok-2-image' || motor.id === 'grok-imagine-video') ? 'XAI_API_KEY'
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
  ...
  async function playGrokVideo(s) {
    ...
    if (!confirmPro('Grok Imagine (xAI)', `~${duration}s a 720p · consume créditos xAI`)) return;
    showPlayer(`
      <div class="player-card">
        <div class="player-head">▶ VIDEO · Grok Imagine · ${aspect} · ${duration}s · 720p</div>
        <pre class="player-body player-loading" id="grokStatus">// enviando prompt a api.x.ai ...</pre>
      </div>`);
    try {
      ...
      while (true) {
        ...
        const pollR = await fetch(`https://api.x.ai/v1/videos/${request_id}`, {

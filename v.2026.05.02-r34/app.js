(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r34';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r34';
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
  ...
  function bindXtoreBadge() {
    ...
    el.addEventListener('click', () => {
      window.open('/pixer-ai-content/signage.html', '_blank');
    });
    refreshXtoreStatus();
    setInterval(refreshXtoreStatus, 10000);
  }

  // ─── Importar desde URL (yt-dlp vía suno-local) ─────────────────
  function bindImportModal() {
    ...
    document.getElementById('doImport')?.addEventListener('click', async () => {
      ...
      stat.textContent = `// llamando a localhost:3777/yt/import (${fmt})...\n// puede tardar 10-60s según video`;
      const t0 = Date.now();
      try {
        const r = await fetch('http://localhost:3777/yt/import', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (player) {
          ...
          player.innerHTML = `
            <div class="player-card">
              <div class="player-head">📥 IMPORTADO · ${kind.toUpperCase()} · ${sizeMB} MB · ${sec}s</div>
              ...
              <small class="player-foot">// vía yt-dlp local · ahora puedes pulsar 📺 ENVIAR A ADMIRA XP</small>
            </div>`;
        }
        stat.textContent = `✓ Importado (${sizeMB} MB en ${sec}s) · listo en el player`;

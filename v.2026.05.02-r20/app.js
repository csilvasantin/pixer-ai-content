(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r20';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r20';
  ...
  function hasKeyFor(motorId) {
    ...
    if (motorId === 'grok-2-image' || motorId === 'grok-imagine-video') return true; // proxied vía worker
  ...
  const MOTORES = {
    ...
    imagenes: [
      ...
      { id: 'grok-2-image',  nombre: 'Grok 2 Image (xAI)', tipo: 'pro', coste: '$0.07 / imagen',         desc: 'vía worker · sin key cliente' },
    ...
    document.querySelectorAll('[data-motor-section]').forEach(host => {
      ...
      function renderWarning() {
        ...
        const keyLabel = motor.id === 'elevenlabs-v2' ? 'WORKER pixer-eleven'
                       : (motor.id === 'grok-2-image' || motor.id === 'grok-imagine-video') ? 'WORKER pixer-eleven'
    ...
    if (motor === 'elevenlabs-v2') {
      ...
      showPlayer(`
        <div class="player-card">
          ...
          <pre class="player-body player-loading">// llamando a pixer-eleven.csilvasantin.workers.dev ...</pre>
        </div>`);
      try {
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
  ...
  async function playImagenes() {
    ...
    if (motor === 'grok-2-image') {
      if (!confirmPro('Grok 2 Image (xAI)', '$0.07 / imagen · vía worker pixer-eleven')) return;
      showPlayer(`
        <div class="player-card">
          <div class="player-head">▶ IMAGEN · Grok 2 Image (xAI)</div>
          <pre class="player-body player-loading">// llamando a pixer-eleven.csilvasantin.workers.dev/xai/image ...</pre>
        </div>`);
      try {
        const r = await fetch(XAI_WORKER_URL + '/xai/image', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ prompt: fullPrompt, n: 1 }),
        });
        if (!r.ok) {
  ...
  async function playGrokVideo(s) {
    ...
    showPlayer(`
      <div class="player-card">
        ...
        <pre class="player-body player-loading" id="grokStatus">// enviando prompt al worker ...</pre>
      </div>`);
    try {
      ...
      while (true) {
        ...
        if (status) status.textContent = `// generando... ${elapsed}s · intento ${attempt} · request ${request_id.slice(0, 8)}`;
        ...
        if (poll.status === 'done' && poll.video?.url) {
          showPlayer(`
            <div class="player-card">
              ...
              <small class="player-foot">// xAI · ${(poll.video.duration || duration)}s · 720p · ${elapsed}s de procesado</small>
            </div>`);
          return;
        }

(function () {
  ...
  async function playLyria(s, model) {
    ...
    if (!(await confirmPro('Lyria 2 (Google)', '~$0.06 por sample · vía worker pixer-eleven (GCP)'))) return;
    showPlayer(`
      <div class="player-card">
...
// Pentatónica Cm · ${totalBeats} beats · ${(beat * totalBeats).toFixed(1)}s</pre>
  ...
  async function compareAllImages(s, fullPrompt, w, h) {
    if (!(await confirmPro('TODAS las imágenes', '~$0.19 total · 5 motores en paralelo (Pollinations gratis + Imagen 4 + Imagen 4 Ultra + Grok + Grok Pro)'))) return;
    const aspectRatio = ASPECT_IMAGEN[s.encuadre] || '1:1';
    const motors = [
  ...
  async function playGrokVideo(s) {
    ...
    if (!(await confirmPro('Grok Imagine (xAI)', `~${duration}s a 720p · vía worker pixer-eleven`))) return;

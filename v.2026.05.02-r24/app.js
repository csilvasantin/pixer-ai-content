(function () {
  const STORAGE_KEY = 'pixer-brief-v.2026.05.02-r24';
  const KEYS_STORE = 'pixer-keys';
  const VERSION = 'v.2026.05.02-r24';
  ...
  function hasKeyFor(motorId) {
    ...
    if (motorId === 'lyria-002' || motorId === 'lyria-3' || motorId === 'lyria-3-pro') return true; // proxied vía worker
  ...
  const MOTORES = {
    ...
    musica: [
      ...
      { id: 'lyria-002',  nombre: 'Lyria 2 (Google)',       tipo: 'pro',  coste: '~$0.06 / sample',     desc: '30-60s estéreo 48kHz · vía worker' },
      { id: 'suno-local', nombre: 'Suno (local)',           tipo: 'pro',  coste: 'bloqueado captcha',   desc: 'pendiente 2Captcha integration' },
    ],
    imagenes: [
    ...
    document.querySelectorAll('[data-motor-section]').forEach(host => {
      ...
      function renderWarning() {
        ...
        const keyLabel = motor.id === 'elevenlabs-v2' ? 'WORKER pixer-eleven'
                       ...
                       : (motor.id === 'lyria-002' || motor.id === 'lyria-3' || motor.id === 'lyria-3-pro') ? 'WORKER pixer-eleven (GCP)'
